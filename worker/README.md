...
**지난 결과 불러오기 (`GET /results`, `GET /results/<날짜>`)**
- `GET /results`는 `results/` 폴더에 저장된 모든 날짜 목록을 최신순으로 반환합니다 (`{ "dates": ["2026-09-14", "2026-08-09", ...] }`).
- `GET /results/<YYYY-MM-DD>`는 그 날짜에 커밋된 결과 텍스트를 그대로 반환합니다 (`{ "date": "...", "content": "..." }`, 없으면 404).
- 페이지의 "지난 결과 불러오기" 카드가 이 두 엔드포인트로 날짜 목록을 채우고, 선택한 날짜의 저장된 내용을 그대로 보여줍니다.
